...
def load_dataset():
    train_dataset = h5py.File('datasets/train_trick11a.h5', "r")
    train_set_x_orig = np.array(train_dataset["train_set_x"][:])  # your train set features
    train_set_y_orig = np.array(train_dataset["train_set_y"][:])  # your train set labels

    test_dataset = h5py.File('datasets/test_trick11a.h5', "r")
...
def initialize_parameters():
    ...
    W3 = tf.get_variable("W3", [ny, 12], dtype=tf.float64,initializer=tf.contrib.layers.xavier_initializer(seed=1))
    b3 = tf.get_variable("b3", [ny, 1], dtype=tf.float64,initializer=tf.zeros_initializer())
...
with tf.Session() as sess:
    ...
    print("the ML prediction after Softmax: %s" % aa.eval())
    print("the ML prediction is: %s" % pp.eval())
    cc=Y_test[:,index]
    key=cc.argmax()
    print("the correct answer is %s" % key, "[1: one; 0: three; 2: other]")